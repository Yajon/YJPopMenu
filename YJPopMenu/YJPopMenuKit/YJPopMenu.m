...
#define kYJPopMenuTag 20160801

#pragma mark - /////////////////
#pragma mark - private categorys

@interface UIView (GYJPopFrame)
...
@implementation UIView (GYJPopFrame)
...
@interface YJPopMenuCell : UITableViewCell
@property (nonatomic, assign) BOOL isShowSeparator;
@property (nonatomic, strong) UIColor * separatorColor;
@end

@implementation YJPopMenuCell
...
#pragma mark - ///////////
#pragma mark - YJPopMenu

@interface YJPopMenu ()<UITableViewDelegate,UITableViewDataSource>
@end

@implementation YJPopMenu
{
...
{
    static YJPopMenu *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        YJPopMenu *popMenuView = [[UIApplication sharedApplication].keyWindow viewWithTag:kYJPopMenuTag];
        if (popMenuView) {
            _sharedInstance = popMenuView;
        }else{
            _sharedInstance = [[YJPopMenu alloc]init];
        }
    });
                      ...
                      delegate:(id<YJPopMenuDelegate>)delegate
{
    if (self = [super init]) {
        ...
        _icons  = [NSMutableArray arrayWithArray:icons];
        _dismissOnSelected = YES;
        _fontSize = 15.0;
        _textColor = [UIColor blackColor];
        _offset = 0.0;
        _type = YJPopMenuTypeDefault;
        _contentColor = [UIColor whiteColor];
        _separatorColor = [UIColor lightGrayColor];
        
        if (delegate) self.delegate = delegate;
        
        self.identifier = identifier?identifier:nil;
...
- (void)dismiss
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(popMenuBeganDismiss)]) {
        [self.delegate popMenuBeganDismiss];
    }
    [UIView animateWithDuration: 0.25 animations:^{
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(popMenuDidDismiss)]) {
            [self.delegate popMenuDidDismiss];
        }
        self.delegate = nil;
...
+ (instancetype)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth identifier:(NSString *)identifier delegate:(id<YJPopMenuDelegate>)delegate
{
    YJPopMenu *popupMenu = [[YJPopMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth identifier:identifier delegate:delegate];
    [popupMenu showAtPoint:point];
    return popupMenu;
}

+ (instancetype)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth identifier:(NSString *)identifier delegate:(id<YJPopMenuDelegate>)delegate
{
    YJPopMenu *popupMenu = [[YJPopMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth identifier:identifier delegate:delegate];
...
{
    static NSString * identifier = @"GYJPopMenuCell";
    YJPopMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[YJPopMenuCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(popMenuDidSelectedAtIndex:title:popMenu:)]) {
        
        [self.delegate popMenuDidSelectedAtIndex:indexPath.row title:_titles[indexPath.row] popMenu:self];
    }
}
...
{
    YJPopMenuCell *cell = [self getLastVisibleCell];
...
- (YJPopMenuCell *)getLastVisibleCell
{
...
- (void)setType:(YJPopMenuType)type
{
    _type = type;
    switch (type) {
        case YJPopMenuTypeDark:
        {
